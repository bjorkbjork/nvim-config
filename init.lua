...
require('lazy').setup({
  ...
  },
  {
    ...
    dependencies = {
      -- LSP servers themselves are provided by the nix flake at flake.nix
      -- (on $PATH via `nix profile add ~/.config/nvim`). No Mason needed.
    ...
    config = function()
      ...
      -- Wire each server in the `servers` table above directly via lspconfig.
      -- Binaries are expected on $PATH — see flake.nix for what gets installed.
